...
public class Keys {
    ...
    protected int[][] generateKeyMatrix(int roundNo){
        ...
        switch (algorithm){
            case 128:
                ...
                for (int i = 0; i < 4; i++) currentKeyMatrix[i][0] ^= column[i];
            ...
            case 256:
                switch (roundNo%2){
                    ...
                    case 1:
                        ...
                        //STEP 2.3.2 : GENERATE THE FIRST COLUMN OF THE NEXT KEY MATRIX
                        if(roundNo!=1) column = functionG(column);
                        for (int i = 0; i < 4; i++)
    ...
    private int[] functionG(int[] column){
        ...
        for (int i = 1; i <4 ; i++) resultantKeyColumn[i-1] = column[i];

        //STEP 2 : SUBBYTES
        SBox S = new SBox('e');
        for (int i = 0; i < 4 ; i++) resultantKeyColumn[i] = S.performSubstitution(resultantKeyColumn[i]);
