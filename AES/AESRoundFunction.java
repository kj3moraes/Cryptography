...
public class AESRoundFunction {
    ...
    protected static void mixColumns(int[][] matrix){
        for (int i = 0; i < 4; i++) {
            matrix[0][i] = dotProduct(2,matrix[0][i]) ^
                    dotProduct(3,matrix[1][i]) ^ matrix[2][i] ^ matrix[3][i];

            matrix[1][i] = matrix[0][i] ^ dotProduct(2,matrix[1][i]) ^
                    dotProduct(3,matrix[2][i]) ^ matrix[3][i];

            matrix[2][i] = matrix[0][i] ^ matrix[1][i] ^
                    dotProduct(2,matrix[2][i]) ^ dotProduct(3,matrix[3][i]);
